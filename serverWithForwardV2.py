...
def data_route():
    ...
    try:
        ...
        data_tuple = (
            ...
            int(float(data.get("satcount")))
            if data.get("satcount") not in (None, "", "None")
            else None,
...
@app.route("/command/cancel", methods=["GET", "POST"])
def cancel_route():
    """Cancel/reset the currently queued or executing command.

    POST: raises the cancel flag and clears any queued command, so the
    control script (postalt.py) can break out of blocking wait loops
    (e.g. waiting for GUIDED mode, or flying to a waypoint) and immediately
    accept the next command.

    GET: consumes (reads and clears) the cancel flag. This is intended to
    be polled by the control script inside its blocking loops.
    """
    global current_command, cancel_flag
    ...
    if request.method == "POST":
        ...
        with command_lock:
            current_command = {"command": ""}
        logger.info("Cancel requested; command queue cleared")
        return jsonify({"message": "cancel requested"}), 200

    # GET: consume the flag atomically
    with cancel_lock:
        flag = cancel_flag["cancel"]
        cancel_flag["cancel"] = False
    return jsonify({"cancel": flag}), 200
...
def command_route():
    ...
    if request.method == "GET":
        # Consume-and-clear so the control script doesn't keep re-executing
        # the same stale command on every poll.
        with command_lock:
            cmd = current_command
            current_command = {"command": ""}
        return jsonify(cmd)
    ...
    try:
        ...
        if len(command) > MAX_COMMAND_LENGTH:
            return jsonify(
                {"error": f"command too long (max {MAX_COMMAND_LENGTH})"}
            ), 400
        ...
        if command.startswith("testmotor"):
            ...
            try:
                ...
                if not (1 <= motor_num <= 8 and 0 <= throttle <= 100):
                    return jsonify(
                        {"error": "Invalid motor number or throttle value"}
                    ), 400
            except ValueError:
            ...
            try:
                ...
                if len(parts) == 2:
                    ...
                    return jsonify(
                        {
                            "error": "Invalid forward command format. Use forward,<distance> or forward,<x>,<y>,<z>"
                        }
                    ), 400

                if not (
                    -FORWARD_XY_LIMIT <= x <= FORWARD_XY_LIMIT
                    and -FORWARD_XY_LIMIT <= y <= FORWARD_XY_LIMIT
                    and -FORWARD_Z_LIMIT <= z <= FORWARD_Z_LIMIT
                ):
                    return jsonify(
                        {
                            "error": f"Invalid forward value. Allowed: x/y in [-{FORWARD_XY_LIMIT}, {FORWARD_XY_LIMIT}], z in [-{FORWARD_Z_LIMIT}, {FORWARD_Z_LIMIT}]"
                        }
                    ), 400
            except ValueError:
            ...
            try:
                altitude = float(parts[1])
                if not (0.1 <= altitude <= 100):
                    return jsonify(
                        {"error": "Invalid takeoff altitude. Allowed 0.1..100m"}
                    ), 400
            except ValueError:
